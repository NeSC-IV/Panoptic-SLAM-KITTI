...
Sophus::SE3f Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp, string filename)
{
    ...
    if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET || (lastID - initID) < mMaxFrames)
    {
        mCurrentFrame.undistort(mImGray,timestamp,mpIniORBextractor,mpORBVocabulary,mpCamera,mDistCoef,mbf,mThDepth,&mLastFrame);
        std::cout<< "Frame ID: " << mCurrentFrame.mnId << "\n";
    ...
    else
    {
        cv::Mat people_mask,mask_temp;
        // people_mask = cv::Mat::zeros(cv::Size(640, 480), CV_8UC3);
        people_mask = cv::Mat::zeros(cv::Size(1241, 376), CV_8UC3);
...
Sophus::SE3f Tracking::GrabImageMonocularOrigin(const cv::Mat &im, const double &timestamp, string filename)
{
    mImGray = im;
    if(mImGray.channels()==3)
    {
...
void Tracking::Thing_Stuff_Detachment()
{
    ...
    for (unsigned int j=0; j< current_panoptic_objs.size(); j++) // for every panoptic mask
    {
        ...
        if(mask_obj.isThing && mask_obj.category_id==2) {continue;} // ignore people
