...
{
    // uchar *data1 = (uchar *)PyByteArray_AsString(pData);
    // cv::Mat img1(rows, cols, CV_8UC3, data1);
    // mask = img1.clone();
    if (pData == NULL)
    {
...
cv::Mat PanopticNet::GetPanoptic_image()
{
    ...
    PyNumpy_AsCV(pValue,376,1241, img);
    Py_DECREF(pValue);
    return img;
...
Panoptic_image PanopticNet::GetPanoptic()
{
        ...
        for(Py_ssize_t i = 0; i < PyList_Size(pValue); i++) 
        {
            ...
            for(Py_ssize_t j=0; j<PyList_Size(value); j++) //[id,isThing,score,category_id,instance_id,area,mask,bbox]
            {
                ...
                switch (j)
                {
                    ...
                    case Element::mask_elem:     {PyNumpy_AsCV(sub_value,376,1241,mask); break;}
    ...
    std::cout << "begin detect3" << endl;
    gstate = PyGILState_Ensure();
    PyObject *pValue1 = PyEval_CallObject(this->py_get_instance_mask,NULL);
    if (PyErr_Occurred())
    {
        PyErr_Print();  // 打印错误信息
    }
    PyGILState_Release(gstate);
    if(pValue1 == NULL) {std::cout<<"Error in python code \n";}
    cv::Mat img1;
    // PyNumpy_AsCV(pValue,480,640, img);
    PyNumpy_AsCV(pValue1,376,1241, img1);
    Py_DECREF(pValue1);
    results.union_instance_mask = img1;
    // results.union_instance_mask = GetUnion_Instance_Mask();

    //get output image
    gstate = PyGILState_Ensure();
    PyObject *pValue2 = PyEval_CallObject(this->py_get_image,NULL);
    if (PyErr_Occurred())
    {
        PyErr_Print();  // 打印错误信息
    }
    ...
    cv::Mat img2;
    // PyNumpy_AsCV(pValue,480,640, img); 
    PyNumpy_AsCV(pValue2,376,1241, img2);
    Py_DECREF(pValue2);
    results.image = img2;
    // results.image = GetPanoptic_image();

    gstate = PyGILState_Ensure();
    PyObject *pValue3 = PyEval_CallObject(this->py_get_all_masks,NULL);
    if (PyErr_Occurred())
    {
        PyErr_Print();  // 打印错误信息
    }
    ...
    cv::Mat img3;
    // PyNumpy_AsCV(pValue,480,640,img);
    PyNumpy_AsCV(pValue3,376,1241,img3);
    Py_DECREF(pValue3);
    results.all_masks = img3;
    ...
    std::cout << "begin detect3 ok" << endl;
